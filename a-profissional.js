...
document.getElementById('btnCalculateProfessional').onclick = function() {
  ...
  if (temBlocoC) {
    ...
    // CAPTURA DO GÊNERO (Esperado: "M" para Masculino, "F" para Feminino)
    const genero = document.getElementById('prof_genero')?.value || "M"; 

    // --- DICIONÁRIO DE CONFIGURAÇÃO DE LIMITES POR GÊNERO (CRIAÇÃO DOS DOIS CASOS DE VARIÁVEIS) ---
    const ref = {
      M: {
        crLeveMin: 1.3, crLeveMax: 1.5, crModMin: 1.6, crModMax: 2.0, crBaixa: 0.4,
        tgpNormal: 50, tgpLeveMin: 51, tgpLeveMax: 150, tgpModMin: 151, tgpModMax: 500,
        tgoLeveMin: 41, tgoLeveMax: 120, tgoModMin: 121, tgoModMax: 400, tgoMax: 400,
        ggtNormal: 60, ggtLeveMin: 61, ggtLeveMax: 120, ggtModMin: 121, ggtModMax: 300, ggtGrave: 300,
        faLeveMin: 131, faLeveMax: 195, faModMin: 196, faModMax: 390, faMax: 390
      },
      F: {
        crLeveMin: 1.1, crLeveMax: 1.3, crModMin: 1.4, crModMax: 1.7, crBaixa: 0.4,
        tgpNormal: 35, tgpLeveMin: 36, tgpLeveMax: 105, tgpModMin: 106, tgpModMax: 350,
        tgoLeveMin: 33, tgoLeveMax: 96, tgoModMin: 97, tgoModMax: 320, tgoMax: 320,
        ggtNormal: 43, ggtLeveMin: 44, ggtLeveMax: 86, ggtModMin: 87, ggtModMax: 215, ggtGrave: 215,
        faLeveMin: 106, faLeveMax: 157, faModMin: 158, faModMax: 315, faMax: 315
      }
    }[genero];
    ...
    if (!isNaN(creatinina) || !isNaN(ureia) || !isNaN(acidoUrico)) {
      if (!isNaN(creatinina)) {
        ...
        } else if (creatinina >= ref.crModMin && creatinina <= ref.crModMax) {
        ...
        } else if (creatinina >= ref.crLeveMin && creatinina <= ref.crLeveMax) {
          renalStatus = "Disfunção Renal Inicial / Leve";
          renalColor = "#ffa500";
          blocoCHipotese.push(`Perda Inicial de Função Filtrante Renal / Insuficiência Pré-Renal Leve (Marcador elevado para o gênero ${genero === "F" ? "Feminino" : "Masculino"})`);
        ...
        } else if (creatinina < ref.crBaixa && creatinina > 0) {
          renalStatus = "Creatinina Sérica Reduzida";
          renalColor = "#00bfff";
          blocoCHipotese.push(`Creatinina Baixa (${creatinina} mg/dL): Compatível com Sarcopenia, Desnutrição Crônica ou Severa Redução de Massa Muscular para o padrão do gênero`);
        }
      }
      ...
      if (!isNaN(ureia) && !isNaN(creatinina) && creatinina > 0) {
        ...
        } else if (razaoUC >= 10 && razaoUC <= 20 && creatinina > ref.crLeveMin) {
    ...
    if (!isNaN(tgo) || !isNaN(tgp) || !isNaN(gamagt) || !isNaN(fosfatase) || !isNaN(biliTotal) || !isNaN(biliDireta)) {
      if (!isNaN(tgo) || !isNaN(tgp)) {
        ...
        if (tgo > ref.tgoMax || tgp > ref.tgpModMax) {
        ...
        } else if ((tgo >= ref.tgoModMin && tgo <= ref.tgoModMax) || (tgp >= ref.tgpModMin && tgp <= ref.tgpModMax)) {
        ...
        } else if ((tgo >= ref.tgoLeveMin && tgo <= ref.tgoLeveMax) || (tgp >= ref.tgpLeveMin && tgp <= ref.tgpLeveMax)) {
          hepaticaStatus = "Alteração Enzimática Leve";
          hepaticaColor = "#ffa500";
          blocoCHipotese.push(`Estresse Hepático Inicial (Compatível com Esteatose Hepática / Infiltração Gordurosa Difusa baseado no corte de TGP > ${ref.tgpNormal} U/L)`);
      ...
      if (!isNaN(gamagt) || !isNaN(fosfatase)) {
        if (gamagt > ref.ggtGrave || fosfatase > ref.faMax) {
        ...
        } else if ((gamagt >= ref.ggtLeveMin && gamagt <= ref.ggtModMax) || (fosfatase >= ref.faLeveMin && fosfatase <= ref.faModMax)) {
          blocoCHipotese.push(`Elevação Isolada de Enzimas Canaliculares (Indução por medicamentos, álcool ou obstrução parcial inicial para o gênero)`);
      ...
      if (!isNaN(biliTotal)) {
        ...
        } else if (biliTotal >= 2.6 && biliTotal <= 5.0) {
          blocoCHipotese.push("Hiperbilirrubinemia Moderada");
        } else if (biliTotal >= 1.3 && biliTotal <= 2.5) {
          blocoCHipotese.push("Hiperbilirrubinemia Leve (Início de icterícia clínica ou subclínica)");
        }

        if (biliTotal > 1.2) {
          if (!isNaN(biliDireta) && biliDireta > 0.3) {
            if (biliDireta > 3.0) {
              blocoCHipotese.push("Predomínio Conjugado Severo (Direto): Reforça quadro de Obstrução Biliar Total, Lesão Parenquimatosa ou Colestase Grave");
            } else {
    ...
    if (!isNaN(sodio) || !isNaN(potassio) || !isNaN(cloro) || !isNaN(bicarbonato)) {
      if (!isNaN(potassio)) {
        if (potassio < 3.5) {
          ...
          } else if (potassio >= 2.5 && potassio <= 2.9) {
            eletroliticoStatus = "Instabilidade Moderada (Hipocalemia)";
            eletroliticoColor = "#ff4d4d";
            blocoCHipotese.push("Hipocalemia Moderada Detectada");
          } else {
            eletroliticoStatus = "Instabilidade Leve (Hipocalemia)";
            eletroliticoColor = "#ffa500";
            blocoCHipotese.push("Hipocalemia Leve Detectada");
          }
        } else if (potassio > 5.0) {
          if (potassio > 6.0) {
          ...
          } else if (potassio >= 5.6 && potassio <= 6.0) {
            eletroliticoStatus = "Instabilidade Moderada (Hipercalemia)";
            eletroliticoColor = "#ff4d4d";
            blocoCHipotese.push("Hipercalemia Moderada Detectada");
          } else {
            eletroliticoStatus = "Instabilidade Leve (Hipercalemia)";
            eletroliticoColor = "#ffa500";
            blocoCHipotese.push("Hipercalemia Leve Detectada");
          }
        }
      }
      ...
      if (!isNaN(sodio)) {
        if (sodio < 135) {
          if (sodio < 125) {
            eletroliticoStatus = "Instabilidade Crítica (Hiponatremia Severa)";
            eletroliticoColor = "#ff0000";
            blocoCHipotese.push("Hiponatremia Severa/Crítica: Risco iminente de complicações neurológicas");
          } else if (sodio >= 125 && sodio <= 129) {
            if (eletroliticoColor !== "#ff0000") { eletroliticoStatus = "Hiponatremia Moderada"; eletroliticoColor = "#ff4d4d"; }
            blocoCHipotese.push("Hiponatremia Moderada Detectada");
          } else {
            if (eletroliticoColor !== "#ff0000") { eletroliticoStatus = "Hiponatremia Leve"; eletroliticoColor = "#ffa500"; }
            blocoCHipotese.push("Hiponatremia Leve Detectada (Sinal de Intoxicação por Água, Excesso Hídrico Relativo ou Perda Soluta)");
          }
        } else if (sodio > 145) {
          if (sodio > 160) {
            eletroliticoStatus = "Instabilidade Crítica (Hipernatremia Severa)";
            eletroliticoColor = "#ff0000";
            blocoCHipotese.push("Hipernatremia Severa/Crítica: Desidratação celular grave");
          } else if (sodio >= 151 && sodio <= 159) {
            if (eletroliticoColor !== "#ff0000") { eletroliticoStatus = "Hipernatremia Moderada"; eletroliticoColor = "#ff4d4d"; }
            blocoCHipotese.push("Hipernatremia Moderada Detectada");
          } else {
            if (eletroliticoColor !== "#ff0000") { eletroliticoStatus = "Hipernatremia Leve"; eletroliticoColor = "#ffa500"; }
            blocoCHipotese.push("Hipernatremia Leve Detectada (Sinal Clínico de Desidratação Celular Severa ou Sobrecarga de Sódio)");
          }
        }
      }
      ...
      if (!isNaN(cloro)) {
        if (cloro < 98) {
          blocoCHipotese.push(cloro < 90 ? "Hipocloridemia Severa" : "Hipocloridemia Leve a Moderada");
        } else if (cloro > 107) {
          blocoCHipotese.push(cloro > 115 ? "Hipercloridemia Severa" : "Hipercloridemia Leve a Moderada");
        }
      }

      if (!isNaN(bicarbonato)) {
        if (bicarbonato < 22) {
          blocoCHipotese.push(bicarbonato < 15 ? "Acidose Metabólica Severa (Abaixo de 15 mEq/L)" : "Nivel de Bicarbonato Reduzido (Janela indicativa de Acidose Metabólica em curso)");
        } else if (bicarbonato > 26) {
          blocoCHipotese.push(bicarbonato > 35 ? "Alcalose Metabólica Severa (Acima de 35 mEq/L)" : "Nivel de Bicarbonato Elevado (Janela indicativa de Alcalose Metabólica em curso)");
        }
      }
    } else {
    ...
    htmlBlocoC = `
      <div class="report-section">
        <h4>3. Função renal, hepática e eletrolítica (Laboratório Avançado) - Perfil: ${genero === "F" ? "Feminino" : "Masculino"}</h4>
        ...
        <div class="report-item" style="font-size:0.85rem; padding-left:15px; color:#aaa;">
          <span>Sódio: ${!isNaN(sodio) ? sodio + " mEq/L" : "N/A"} | Potássio: ${!isNaN(potassio) ? potassio + " mEq/L" : "N/A"} | Cloro: ${!isNaN(cloro) ? cloro + " mEq/L" : "N/A"} ${!isNaN(bicarbonato) ? "| Bicarbonato: " + bicarbonato + " mEq/L" : ""}</span>
        </div>
          
        <div style="font-size:0.65rem; color:#666; margin-top:5px; text-align:right;">Ref: Dados Clínicos (2026) | KDIGO | SBPL / RBAC</div>
      </div>`;
}
...
// CAPTURA DO GÊNERO PARA ADEQUAÇÃO TEXTUAL (M ou F)
const generoLaudo = document.getElementById('prof_genero')?.value || "M";
const sufixoGenero = generoLaudo === "F" ? "a" : "o";
const sufixoPlural = generoLaudo === "F" ? "as" : "os";
const textoPerfil = generoLaudo === "F" ? "Paciente: Feminino" : "Paciente: Masculino";

const extra = document.getElementById('professionalExtraContent');
extra.innerHTML = `
    <div class="clinical-report">
      <div style="font-size:0.8rem; color:#aaa; margin-bottom:10px; font-weight:bold; text-transform:uppercase; letter-spacing:0.5px;">
        ${textoPerfil}
      </div>
      ...
      <div class="report-section">
        ...
        <p style="color:#ffa500; font-weight:bold; line-height:1.7; margin-top:5px;">
          ${hipoteses.length > 0 ? hipoteses.join(' | ') : `Nenhuma alteração patológica gerad${sufixoGenero} com as informações inserid${sufixoPlural} até o momento.`}
        </p>
      </div>
      ...
      <div class="conduta-box">
        ...
        <ul style="padding-left:15px; margin-top:5px; font-size:0.9rem; line-height:1.7;">
          ${condutasLista.length > 0 ? condutasLista.map(cond => `<li>${cond}</li>`).join('') : `<li>Sem condutas específicas direcionadas para o perfil selecionado. Insira dados parciais ou totais nos blocos.</li>`}
        </ul>
      </div>
    </div>
`;
